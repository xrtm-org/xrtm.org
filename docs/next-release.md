...
description: Future source work that is not yet on the released 0.9.0 capability set.
---
...
- **Released docs** now target the `xrtm==0.9.0` capability set for the
...
`0.9.0` keeps the bounded local Hub → Studio → Playground → Observatory
...
that is intentionally **not** yet part of the released 0.9.0 claim set:
...
| --- | --- | --- | --- | --- |
| Unified Hub / Studio / Playground / Observatory spine | **`shipped`** | `0.9.0` | Promoted to the released docs/site surface for the release-promotion branch. The public claim is local-first, deterministic by default, and bounded to the existing workflow schema and built-in node catalog. | Keep Gate 1 checks clean for this branch and require release Gate 2 before publication: first-run Hub, template-to-Playground, bounded Studio validate/save/run, Studio-to-Playground trace, Observatory drill-down, and deterministic baseline. |
| WebUI shell polish and trust cues | **`shipped`** | carried forward in `0.9.0` | Version, local-only scope, file-backed history, SQLite draft-state cues, and the locked muted design language remain part of the released shell. | Continue treating these as trust framing, not a hosted app or control-plane claim. |
