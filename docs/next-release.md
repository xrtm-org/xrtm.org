...
description: Near-term conveniences that are not yet on the released 0.8.0 surface.
---
...
- **Released docs** stay pinned to what ships in `xrtm==0.8.0`, because `xrtm` is the product-anchor version for top-level release claims.
...
| WebUI/CLI parity shell | **`shipped`** | `0.8.0` | The released package now ships the local React/TypeScript WebUI shell with Overview, Start, Runs, Workflow detail, Operations, Advanced visibility, run detail, compare, and `/workbench`, with PyPI proof for quickstart, workflow run, report/export, profile/monitor/artifact operations, and guided draft flow. Safe edits are limited to `questions.limit`, the report toggle, and supported aggregate weights. | Keep the public docs honest: Start/Operations parity plus clone, constrained safe edit, validate, run, and compare only; no arbitrary graph, JSON, or code editing claim. |
